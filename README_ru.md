...
1. Объединяет ```Active Record``` модели одной таблицы в дерево по алгоритму ```Materialized path```, с помощью [данного](https://github.com/mgrechanik/yii2-materialized-path) расширения
...
4. По сути вы получите набор ```Active Record``` моделей, организованных в дерево, и ```CRUD``` над ними в **backend** части
...
Если вам не требуются дополнительные поля для ```Active Record``` модели ([подробнее](#custom-ar)), то таблицу для каталога
