...
3) Имеются следующие возможности как указать модулю взять контроллеры от родителя:
    ...
    * ```Views```-ы тоже будет искать в зависимости от настроек выше	
...
#### ```$baseControllerNamespace``` - бпзовое пространство имен для используемых модулем контроллеров
По умолчанию устанавливается в значение пространства имен модуля.  
Но можно изменить на пространство имен любого другого модуля, контроллеры будут браться оттуда
