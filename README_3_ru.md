...
При использовании [данной библиотеке](https://github.com/mgrechanik/image-points-searcher "Библиотека поиска точек на изображении") мы можем загрузить список городов из картинки, и результат поиска отобразить на результирующей картинке. Смотри для примера изображения на [Demo](#demo "картинки на демо, получены этим способом").   

Подробнее по подготовке картинки смотрите в описании к той библиотеке, но вкратце - на белом холсте отметьте кружочками диаметром 10 px, вершины графа, и используйте в коде ниже эту картинку.
...
- Установить текущую выполняемую задачу. TSP или SPP   
```->setTask(Task $task)```
